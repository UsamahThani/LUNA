...
# Function to simulate waiting time with a progress bar (using tqdm)
def get_response_with_progress_bar(duration=5):
    print("Waiting for response...")
    
    # Use tqdm for progress bar
    with tqdm(total=duration, desc="Response Time", ncols=100, unit='s') as pbar:
...
def speak(text):
    # Send text to Piper server and get audio in response
    response = requests.get(PIPER_SERVER_URL, params={"text": text})
    
    if response.status_code == 200:
        # Use progress bar while waiting for the response
        get_response_with_progress_bar(5)  # Simulating a 5-second wait for the response
        ...
        # Add a small delay to ensure the file is no longer in use before deleting
        time.sleep(1)

        # Try to delete the file after use
